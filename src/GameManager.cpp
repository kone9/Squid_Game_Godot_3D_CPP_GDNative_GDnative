...
namespace godot 
{
	...
	void GameManager::_ready()
	{
		...
		Head = (MeshInstance*)get_tree()->get_nodes_in_group("doll_head")[0];//obtengo la cabeza de la mu�eca
	...
	Vector3 GameManager::RandomPosition()
	{
		...
		Vector3 randomCoordinate = origin + randomRange;//posicion de origen m�s posicion aleatoria en el dentro del rectangulo de instancia
	...
	void GameManager::_on_dollSing_finished()
	{
		...
		Timer_active_raycast_detection->set_wait_time(1);//el tiempo de espera es igual al de la rotaci�n de la cabeza
	...
	void GameManager::_on_Timer_Rotate_head_timeout()
	{
		...
		emit_signal("can_Walk");//emite la se�al que se puede volver a caminar
